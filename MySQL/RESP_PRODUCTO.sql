...
CREATE TABLE IF NOT EXISTS `ejemplo_producto`.`COMPRA` (
  ...
  PRIMARY KEY (`CLIENTE_dni`, `PRODUCTO_producto`),
  INDEX `fk_CLIENTE_has_PRODUCTO_PRODUCTO1_idx` (`PRODUCTO_producto` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_CLIENTE_has_PRODUCTO_PRODUCTO1`
    ...
    REFERENCES `ejemplo_producto`.`PRODUCTO` (`producto`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
